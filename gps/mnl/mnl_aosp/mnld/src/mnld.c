...
linux_gps_init (void)
{
	...
	else if(strcmp(chip_id, "0x0321") == 0) //Denali1
	{
		...
		unsigned int devinfo_data = 47;   
		int segment_val = 0;
		if(fd == -1)
	  {
		  MND_MSG("Unable to open - /dev/devmap \n");
		  return MTK_GPS_ERROR;
	  } else {
		  ioctl(fd,READ_DEV_DATA,&devinfo_data);
		  segment_val = devinfo_data;
		  MND_MSG("open /dev/devmap successfully, ioctl segment_val = %x \n",segment_val);
		  close(fd);
	  }
	  
    if((segment_val & 0x22000000) == 0x22000000) // chip is 6735 p/c
    {
    	MND_MSG("chip is 6735 p/c \n");
		  mnl_config.GNSSOPMode = 3; // gps only
    }
    else
    {   	 
			if(((gnss_mode_flag & 0x01000000) != 0) && ((gnss_mode_flag & 0x02000000) == 0)) //  01 
			{
				mnl_config.GNSSOPMode = 3; // gps only
			}
    }

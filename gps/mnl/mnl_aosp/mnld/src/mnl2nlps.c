...
{
    return socket_local_client(MNL_TO_NLPS_SOCKET_NAME, ANDROID_SOCKET_NAMESPACE_ABSTRACT, SOCK_STREAM | SOCK_CLOEXEC | SOCK_NONBLOCK);
}
...
static int close_fd(int fd)
{
    int ret = close(fd);
    if (ret != 0) {
        MND_ERR("[nlp] close fd %d error:%s\n", fd, strerror(errno));
    } else {
        MND_MSG("[nlp] close fd %d ok\n", fd);
    }
    return ret;
}
...
static int mnl2nlp_send_cmd3_and_quit(mnl_nlps_cmd_enum_t cmd, int data1, int data2, int data3)
{
    ...
    MND_MSG("[nlp] socket_fd: %d", socket_fd);
    if (socket_fd < 0) {
        MND_ERR("[nlp] connect error:%s\n", strerror(errno));
        return -1;
    }

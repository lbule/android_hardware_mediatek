...
#ifdef CONFIG_MTK_P2P /* Only return -1 means error */
...
/* [ALPS00440896] connect AP by WPS pin method, can connect successfully but wifiStatemachine 
   return WifiManager.WPS_FAILED */	
#ifdef CONFIG_MTK_P2P
	if (!wps->wps->ap && !wps->er && !wps->registrar)
#else
	if (!wps->wps->ap && !wps->er)
#endif /*Mediatek Modified*/
	wps_sta_cred_cb(wps);

...
//to speed up STAGE_SCAN
#ifdef CONFIG_MTK_STAGE_SCAN
		wpa_supplicant_stage_scan_reschedule(drv->ctx);
#endif
		break;
...
static int wpa_driver_nl80211_shared_freq(void *priv)
{
	...
	dl_list_for_each(driver, &drv->global->interfaces,
			 struct wpa_driver_nl80211_data, list) {
		if (drv == driver ||
#if !defined(CONFIG_MTK_SCC) && !defined(CONFIG_MTK_WFD_LEGACY_COEX) /* P2P vs lagecy wifi multi-channel concurrence */

...
static int setup_interface2(struct hostapd_iface *iface)
{
	...
	if (hostapd_get_hw_features(iface)) {
		...
		if (ret == 2) {
			wpa_printf(MSG_DEBUG, "LTE: MTK_LTE_COEX ACS enable");
		}
#endif		
...
#ifdef CONFIG_MTK_LTE_COEX
static void lte_scan_complete(struct hostapd_iface *iface)
{
	iface->scan_cb = NULL;
	wpa_printf(MSG_DEBUG, "LTE: Scan complete");
}
static int lte_request_scan(struct hostapd_iface *iface)
{
	...
	*freq = 0;
	...
	wpa_printf(MSG_DEBUG, "LTE: Scanning");
...
#ifdef CONFIG_MTK_LTE_COEX
#ifdef CONFIG_P2P
	wpa_printf(MSG_DEBUG, "Skip LTE align channel for P2P GO"); 
#else
	if (iface->conf->channel == 0) {	 
		u8 ret_ch_info;
		wpa_printf(MSG_DEBUG, "LTE align channel"); 
		lte_request_scan(iface);
		ret_ch_info = hostapd_channel_align_LTE_coex(hapd, 6);
		wpa_printf(MSG_DEBUG, "ret_ch_info = %d\n",ret_ch_info); 
		if (ret_ch_info != 0) { /* if ret_ch_info == 0 keep orignial value of hapd->iconf->channel */
			iface->conf->channel = ret_ch_info;
		} else {

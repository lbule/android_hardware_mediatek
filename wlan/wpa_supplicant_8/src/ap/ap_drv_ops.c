...
extern  int wpa_driver_nl80211_testmode(void *priv, const u8 *data,
									   size_t data_len);

int hostapd_drv_get_available_channel(struct hostapd_data *hapd, u8 *buf)
{
	...
	params.buf = buf;
	return wpa_driver_nl80211_testmode(hapd->drv_priv, (u8 *)&params, 

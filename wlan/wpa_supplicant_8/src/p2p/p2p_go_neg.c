			    ...
			    const u8 *data, size_t len, int rx_freq)
{
	...
	/* only report no common channel event in WFD SINK mode, because p2p->channels
	* may be not configured before p2p_connect/p2p_invite
	*/
	if (p2p->wfd_mode == 2 &&
		p2p_peer_channels(p2p, dev, msg.channel_list,
			      msg.channel_list_len) < 0) {
		if (msg.operating_channel) {
			p2p->peer_oper_freq = p2p_channel_to_freq(msg.operating_channel[3],
								 msg.operating_channel[4]);

			p2p_dbg(p2p, "Store peer oper freq from nego req: %d MHz",
				p2p->peer_oper_freq);
		}

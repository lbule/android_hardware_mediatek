...
#include "common.h"
#ifndef CONFIG_GET_IMSI_FROM_PROPERTY
				     ...
				     struct eap_peer_config *conf)
{
	...
	if (conf->sim_slot)
		slot_id = atoi((const char*)conf->sim_slot);
	if (slot_id < 0) /* slot_id < 0 means test with simulated sim */
		return conf->imsi_len > 0 ? 0:-1;
	os_free(conf->imsi);
	conf->imsi_len = 0;
	conf->imsi = os_malloc(60);
	if(conf->imsi) {
		conf->imsi_len = generate_nai((char*)conf->imsi, allowMethod, slot_id);
	}
	return conf->imsi_len > 0 ? 0:-1;
#else /* CONFIG_GET_IMSI_FROM_PROPERTY */	
	return (conf->imsi && os_strcmp((const char*)conf->imsi, "\"none\"") != 0) ? 0:-1;
#endif /* CONFIG_GET_IMSI_FROM_PROPERTY */

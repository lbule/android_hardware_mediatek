...
/* before STA send authentication req, probe request must includes specified ssid */
				(!wpa_s->current_ssid || ssid->scan_ssid)
#else
			    ssid->scan_ssid
#endif
			    ) {
	...
	if (ssid && max_ssids == 1) {
		 ...
		 */
#ifndef CONFIG_MTK_HIDDEN_SSID
/* don't schedule a wildcard scan here, because MTK's driver always do it if supplicant trigger 
    any scan to driver.
    if we don't remove this code, time in UI scan list of APs with hidden ssid will be reduced */

...
{
#ifdef CONFIG_COMPARE_BSS_BY_BSSID_AND_SSID
	struct wpa_ssid *cur_ssid = wpa_s->current_ssid;
#endif
	return bss == wpa_s->current_bss ||
#ifdef CONFIG_COMPARE_BSS_BY_BSSID_AND_SSID
		(os_memcmp(bss->bssid, wpa_s->bssid, ETH_ALEN) == 0 && (!cur_ssid ||
			(cur_ssid && cur_ssid->ssid_len > 0 &&
			os_strncmp(cur_ssid->ssid, bss->ssid, cur_ssid->ssid_len) == 0))) ||
#else
		os_memcmp(bss->bssid, wpa_s->bssid, ETH_ALEN) == 0 ||
#endif
#ifdef CONFIG_MTK_WFD_LEGACY_COEX /* if we're trying to connect this ssid, don't remove it from scan result */

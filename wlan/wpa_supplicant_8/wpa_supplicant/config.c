				...
				const char *value)
{
	...
	if ((ssid->proto & WPA_PROTO_WAPI)!=0) {
        ...
        if(ssid->len_wapi_hex_psk > 64){
			...
			if(ssid->len_wapi_hex_psk%2) return -1;
...
#ifdef CONFIG_MTK_PRESET_SSID
	if (ssid->read_only) {
		wpa_printf(MSG_INFO, ("don't allow removing read only network"));
		return -1;
	}
#endif
		   ...
		   int line)
{
	...
	if (ssid == NULL || var == NULL || value == NULL)
		return -1;
#ifdef CONFIG_MTK_PRESET_SSID
	if (ssid->read_only && os_strncmp(var, "priority", 8) && os_strncmp(var, "disabled", 8)) {
		wpa_printf(MSG_INFO, "forbid modifying %s for read only networks", var);
		return -1;
	}
#endif
...
#ifdef CONFIG_MTK_PRESET_SSID
	{ INT_RANGE(overlay_loaded, 0, 1), 0},
#endif
};

			      ...
			      struct wpa_bss *bss)
{
	...
	struct eap_peer_config *conf = &ssid->eap;
	const char *pos;
	u16 dev_pw_id = 0;

	if (conf->phase1 != NULL) {
		pos = os_strstr(conf->phase1, "dev_pw_id=");
		if (pos) {
			dev_pw_id = atoi(pos + 10);
			//if (dev_pw_id >= 0x10) 
			//	wpa_printf(MSG_DEBUG, "NFC password token(ID=0x%x)", dev_pw_id);
		}
	}
#endif
	if (eap_is_wps_pbc_enrollee(&ssid->eap)) {
		...
		if (wps_ie &&
		    (wps_is_addr_authorized(wps_ie, wpa_s->own_addr, 1) ||
		     (wpa_s->scan_runs >= WPS_PIN_SCAN_IGNORE_SEL_REG && dev_pw_id < 0x10))) {
#else
		if (wps_ie &&
		    (wps_is_addr_authorized(wps_ie, wpa_s->own_addr, 1) ||

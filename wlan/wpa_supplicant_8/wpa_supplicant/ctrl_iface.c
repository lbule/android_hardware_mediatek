					       ...
					       size_t buflen)
{
	...
	if (cmd && os_strlen(cmd)) {
		...
		if (level == MSG_DEBUG) {
			wpa_printf(MSG_WARNING, "Reset log level to EXCESSIVE, as framework want to verbose WiFi log");
...
#ifdef CONFIG_WAPI_SUPPORT
	if (data.key_mgmt & WPA_KEY_MGMT_WAPI_CERT) {
		wpa_printf(MSG_DEBUG,"[Debug] %s => WAPI IS CERT ", __FUNCTION__);
		ret = os_snprintf(pos, end - pos, "%sCERT", pos == start ? "" : "+");
		if (ret < 0 || ret >= end - pos)
			return pos;
		pos += ret;
	}
	if (data.key_mgmt & WPA_KEY_MGMT_WAPI_PSK) {
...
#ifdef CONFIG_MTK_P2P	
	/* ALPS01439965 Flush Bss here, supplicant will re-scan before connect */
	wpa_bss_flush(wpa_s);
#endif
				     ...
				     char *buf, size_t buflen)
{
	int ret;
#ifdef CONFIG_MEDIATEK_WIFI_BEAM
	if(os_strncasecmp(cmd, "BEAMPLUS_", os_strlen("BEAMPLUS_"))==0) {
    	u32 start;
     	struct p2p_data *p2p = wpa_s->global->p2p;

    	if (p2p != NULL) {
	    	if(os_strncasecmp(cmd, "BEAMPLUS_GO_RESERVE_START", os_strlen("BEAMPLUS_GO_RESERVE_START"))==0) {
	    		wpa_printf(MSG_DEBUG, "BEAMPLUS GO Reserve Start");
	    		p2p->in_beam_go_reserve = 1;
	    		wpas_p2p_beamplus_go_reserve_register_timeout(wpa_s);
	    	} else if(os_strncasecmp(cmd, "BEAMPLUS_GO_RESERVE_END", os_strlen("BEAMPLUS_GO_RESERVE_END"))==0) {
	    		wpa_printf(MSG_DEBUG, "BEAMPLUS GO Reserve End");
	    		p2p->in_beam_go_reserve = 0;	
	    	} else {    		
		    	if(os_strncasecmp(cmd, "BEAMPLUS_START", os_strlen("BEAMPLUS_START"))==0)
		    		start = 1;
		    	else if(os_strncasecmp(cmd, "BEAMPLUS_STOP", os_strlen("BEAMPLUS_STOP"))==0) {
		    		start = 0;
		    		p2p->in_beam_go_reserve = 0;
		    	}	
		
		    	wpa_printf(MSG_DEBUG, "Driver Beamplus %s", start==1? "START":"STOP"); 
		    	wpa_drv_driver_cmd(wpa_s,"set_beamplus", (char *)&start, sizeof(u32));
	    	}
	    	ret = 0;
    	}else{
		wpa_printf(MSG_DEBUG, "Driver Beamplus: can't continue because p2p is null");
	}
	return ret;
...
int old_ap_scan = 1;
int stage_scan_occur = 0;
void (*scan_handler)(struct wpa_supplicant *wpa_s,
				 struct wpa_scan_results *scan_res);


static void wpa_supplicant_req_full_band(
	struct wpa_supplicant *wpa_s)
{
	char reply[32];
	...
	reply_len = wpa_supplicant_driver_cmd(wpa_s, "SET_BAND_DUAL", reply,
						      reply_size);						      
}

void wpa_supplicant_req_5g_scan(
	struct wpa_supplicant *wpa_s)
{
	char reply[32];
	const int reply_size = 32;
	...
	if (wpa_s->wpa_state >= WPA_ASSOCIATING) {
		wpa_printf(MSG_DEBUG, "[STAGE_SCAN] Skip 5G scan");
		eloop_cancel_timeout(wpa_supplicant_req_full_band, wpa_s, NULL);
		eloop_register_timeout(0, 0, wpa_supplicant_req_full_band, wpa_s, NULL);
		return;
	}	 
	reply_len = wpa_supplicant_driver_cmd(wpa_s, "SET_BAND_5G", reply,
						      reply_size);
	
	wpa_s->normal_scans = 0;
	wpa_s->scan_req = 2;
	//overwritten ap_scan to comply with 2.4G
	wpa_s->conf->ap_scan = old_ap_scan;
        wpa_s->scan_res_handler = scan_handler;
	wpa_supplicant_req_scan(wpa_s, 0, 0);
	
	wpa_printf(MSG_DEBUG, "[STAGE_SCAN] Schedule full band recovery");
	eloop_cancel_timeout(wpa_supplicant_req_full_band, wpa_s, NULL);
	eloop_register_timeout(1, 0, wpa_supplicant_req_full_band, wpa_s, NULL);
						      
}
void wpa_supplicant_stage_scan_reschedule(void *ctx)
{
	struct wpa_supplicant *wpa_s = ctx;

		if (eloop_is_timeout_registered(wpa_supplicant_req_5g_scan, wpa_s, NULL)) {
				eloop_cancel_timeout(wpa_supplicant_req_5g_scan, wpa_s, NULL);
				wpa_printf(MSG_DEBUG, "[STAGE_SCAN] 5G scan speed up");
				eloop_register_timeout(0, 0, wpa_supplicant_req_5g_scan, wpa_s, NULL);
		}	
}
static int wpa_supplicant_ctrl_iface_scan_progress(
	struct wpa_supplicant *wpa_s, char *reply, size_t reply_size)
{
	int reply_len; 
	 
	reply_len = wpa_supplicant_driver_cmd(wpa_s, "SET_BAND_2G4", reply,
						      reply_size);
	//keep the same ap_scan policy for 2.4G and 5G 
	old_ap_scan = wpa_s->conf->ap_scan;					      
        scan_handler = wpa_s->scan_res_handler;
	wpa_supplicant_req_scan(wpa_s, 0, 0);
						      
	wpa_printf(MSG_DEBUG, "[STAGE_SCAN] Schedule 5G scan request(with scan_handler callback): 1 sec");
	eloop_cancel_timeout(wpa_supplicant_req_5g_scan, wpa_s, NULL);
	eloop_register_timeout(1, 0, wpa_supplicant_req_5g_scan, wpa_s, NULL);

	return reply_len;
}
void wpa_supplicant_req_stage_scan(struct wpa_supplicant *wpa_s, int sec, int usec)
{
	char reply[32];
	const int reply_size = 32;
	int reply_len; 
			   ...
			   char *reply, int reply_size, int *reply_len)
{
	...
	if (!wpa_s->sched_scanning && !wpa_s->scanning &&
		...
		if ((wpa_s->hw.num_modes == 3) && (stage_scan_occur == 0)) {
			stage_scan_occur = 1;
			wpa_printf(MSG_DEBUG, "wpa_s->hw.num_modes = %d", wpa_s->hw.num_modes);
			wpa_printf(MSG_DEBUG, "[STAGE_SCAN] Start stage_scan");
			wpa_supplicant_ctrl_iface_scan_progress(wpa_s, reply, reply_size);
		} else {
			wpa_supplicant_req_scan(wpa_s, 0, 0);
		}
#else					

...
#ifdef CONFIG_MTK_PRESET_SSID
	if (config->overlay_loaded == 1)
		fprintf(f, "overlay_loaded=1");
#endif
}
...
#ifdef CONFIG_MTK_WRITE_FILE
/*ALPS01293874:  when configuration file are being writed then pull out battery quickly, configuration file will be flushed after restart*/
    fflush(f);
    fsync(fileno(f));
#endif
	fclose(f);

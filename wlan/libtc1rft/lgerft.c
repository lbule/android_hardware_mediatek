...
bool LGE_RFT_GetMACAddr(unsigned char *macAddr)
{
    ...
    return true;
#else // defined(USE_LGE_NVRAM)
	//return 0;//MP Branch needs to use this func.LGE_FacWriteWifiMacAddr(macAddr, true);
	return TC1_FAC_NAME(FacReadWifiMacAddr)(macAddr);
#endif
...
bool LGE_RFT_SetMACAddr(unsigned char *macAddr)
{
	...
	return TC1_FAC_NAME(FacWriteWifiMacAddr)(macAddr,true);
...
//for the OTA  test
bool LGE_RFT_AutoTxPower(bool enable)
{
	bool ret = false;
	...
	if(false == enable){  //disable 
		if(0 == wifi_test_set_swctrl(0xa0210000, 0x0000786A, NULL, NULL))
			ret = true;
	}else if(true == enable){ // enable 
		if(0 == wifi_test_set_swctrl(0xa0210000, 0xF4FA786A, NULL, NULL))
			ret = true;
	}

...
#define	WCN_LAUNCHER_READY_PROP "service.wcn.driver.ready"
...
static void* launcher_pwr_on_chip(void * arg)
{
	...
	iRet = property_get(WCN_LAUNCHER_READY_PROP, readyStr, NULL);
	if ((0 != iRet) && (0 == strcmp(readyStr, "yes"))) {
		ALOGI("set property(%s) failed iRet:%d or property is %s\n", WCN_LAUNCHER_READY_PROP, iRet, readyStr);
	} else {
		sprintf (readyStr, "%s", "yes");
		iRet = property_set(WCN_LAUNCHER_READY_PROP, readyStr);
		if (0 != iRet) {
			ALOGI("set property(%s) to %s failed,iRet:%d\n", WCN_LAUNCHER_READY_PROP, readyStr, iRet);
		} else {
			ALOGI("set property(%s) to %s succeed.\n", WCN_LAUNCHER_READY_PROP, readyStr);
		}
	}
...
int main(int argc, char *argv[])
{
	...
	if((0x6735 == chipId) || (0x6752 == chipId) || (0x6582 == chipId) || (0x6592 == chipId) || (0x6572 == chipId) || (0x6571 == chipId) || (0x8127 == chipId) || (0x8163 == chipId) || (0x6580 == chipId))
	{
